...
class TestServer(unittest.TestCase):
    ...
    def test_taskbar_supports_both_chassis_battery_topics(self):
        ...
        self.assertIn("Jetson CPU", html)
        self.assertIn("Jetson RAM", html)
        self.assertIn("busyCores = cpu === null ? null : cpu / 100", javascript)
        self.assertNotIn("const displayedCpu = workloadCpu", javascript)
        self.assertIn("system.cpu_capacity_percent", javascript)
        self.assertIn("status.chassis_telemetry", javascript)

    def test_editor_only_exposes_student_course_packages(self):
        self.assertEqual(
            set(EDITOR_FILES),
            {"ada_high_school", "intro2av_python", "intro2av_cpp"},
        )
        self.assertNotIn("reference", EDITOR_FILES)
        self.assertTrue(all(
            set(files) == {"perception", "planning", "control"}
            for files in EDITOR_FILES.values()
        ))

    def test_editor_opens_intro2av_algorithm_seams_first(self):
        static_dir = Path(__file__).parents[1] / "static"
        javascript = (static_dir / "app.js").read_text(encoding="utf-8")
        self.assertIn(
            "carkit_intro2av/${component}_algorithm.py", javascript
        )
        self.assertIn("src/${component}_algorithm.cpp", javascript)
    ...
    def test_editor_reports_python_syntax_without_losing_saved_work(self):
        with tempfile.TemporaryDirectory() as directory:
            manager = self._editor_manager(directory)
            opened = manager.read_editor_file("ada_high_school", "control")
            saved = manager.save_editor_file(
                "ada_high_school",
                "control",
                "def incomplete(:\n",
    ...
    def test_editor_tree_opens_the_course_package_root(self):
        with tempfile.TemporaryDirectory() as directory:
            manager = self._editor_manager(directory)
            root = Path(directory) / EDITOR_ROOTS["intro2av_python"]
            (root / "README.md").write_text("# Student notes\n", encoding="utf-8")
            tree = manager.editor_tree("intro2av_python")
